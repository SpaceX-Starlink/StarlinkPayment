...
if (checkIncognito()) {
  // 在无痕模式下使用sessionStorage作为临时存储
  orders = JSON.parse(sessionStorage.getItem('orders')) || [];
  localStorage.setItem('orders', JSON.stringify(orders)); // 尝试同步到localStorage
}
...
function addOrder(order) {
  orders.push(order);
  // 同时更新localStorage和sessionStorage
  localStorage.setItem('orders', JSON.stringify(orders));
  sessionStorage.setItem('orders', JSON.stringify(orders));
  broadcast.postMessage({ type: 'update', orders: orders }); // 广播更新
  updateOrderList();
}
...
closeBtn.addEventListener('click', function() {
  if (currentOrderId) {
      ...
      broadcast.postMessage({ type: 'update', orders: updatedOrders }); // 广播更新
